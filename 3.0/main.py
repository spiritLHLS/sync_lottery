...
def input(db, i):
    ...
    if (db_user != None):
        ...
        curd.create_info(user=i, db=db)  # 不在库创建
        return i
...
@application.get("/get_lottery_info/", response_model=List[schemas.Readinfo])
def get_all_info(skip: int = 0, limit: int = 1000, db: Session = Depends(get_db)): # 查找所有用户数据
    users = curd.get_all_info(db, skip=skip, limit=limit)
    return users
